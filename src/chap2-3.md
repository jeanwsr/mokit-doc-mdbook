...
If you do not have the ifort compiler or MKL on your computer, you may want to download and install them. There are several versions recommended, you can choose any one of them:  
...
make all
```
to compile MOKIT. This will take about 2 minutes. There is no `make install`. Assuming that you want to use gfortran+OpenBLAS instead of ifort+MKL, you should run instead
```
...
For Dalton, currently only the MKL version of Dalton is supported. The MPI parallel version will be supported in the future. The environment variable `BDF` are optional, there is no need to write it if you do not want to use BDF.
